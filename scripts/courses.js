...
function calculateCredits(filteredCourses) {
    // Calculate and display number of credits for each course listed
    const initialValue = 0;
    let sumCredits = filteredCourses.reduce((accumulator, courseObject) => {
        return accumulator + courseObject.credits;
    }, initialValue);
    return sumCredits;
}

function displayCourses(filteredCourses) {
    clearDisplayedCourses();
    filteredCourses.forEach((course) => {
        let courseInfo = document.createElement('p');

        courseInfo.innerHTML = ` ${course.subject} ${course.number}`;
        if (course.completed === true) {
            courseInfo.classList.add('complete');
        } else {
